...
-   No site rdrr.io tem o serviço [snippets](https://rdrr.io/snippets/) os códigos do R podem ser executados na internet, o limitante é que não se pode instalar novos pacotes lá e não há um modo privativo e seguro.
...
#### Importância do uso de scripts

Um script é um arquivo de texto que contém um conjunto de comandos ou instruções. Esses comandos e instruções são executados sequencialmente no R.\

::: {.incremental}
-   **Preparação**: para criar esses arquivos é importante para ter um histórico de trabalho.

-   **Arquivo:** O nome do arquivo deve ser suficientemente claro para refletir o conteúdo e encontrá-lo facilmente. Se possível deve ser curto e evitar o uso de `"."` .

-   **Documentação**: É muito importante comentar as operações ou funções utilizadas para poder replicar com facilidade o trabalho. Os comentários devem começar com `"#"`.
:::
...
A lista de pacotes anexados e objetos é alterada com o carregamento de um pacote.
...
??norm # idem anterior
apropos("mean") # Lista de funções que contenham o termo "norm"
...
example(contour) # Executa os ejemplos disponíveis da função
browseVignettes() # Mostra na web as vinhetas disponníveis e o acesso
...
Quando se usa uma função:

-   Os argumentos que tem valores incorretos podem não ser chamados quando a função é executada.
-   Os argumentos tem uma ordem que deve ser seguida se o usuário optar por não nomeá-los.
...
dem <- digeoqR::prepara_dem(dir_in = "inputs/campo/",
                            limite_srtm = "area_srtm.shp",
                            dir_out = "outputs/",
                            EPSG = 4326,
                            z = 11)
